...
function mockDataSyncPlugin() {
  return {
    name: 'admin-mock-data-sync-plugin',
        ...
        if ((req.url === '/api/save-mock-data' || req.url === '/api/sync-data') && req.method === 'POST') {
          ...
          req.on('end', () => {
            try {
              ...
              res.statusCode = 200;
              res.setHeader('Content-Type', 'application/json');
              res.end(JSON.stringify({ success: true, revision: result.revision, message: 'Admin mockData sync completed across portals' }));
            } catch (err) {
              console.error('Error saving mock data in admin:', err);
              res.statusCode = 500;
              res.setHeader('Content-Type', 'application/json');
